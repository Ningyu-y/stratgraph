...
def main():
    ...
    length_text_context = int(12000 * (1.0 - context_threshold))
    print(f"Length of text context: {length_text_context}")
    ...
    embedding_api_key = os.environ["EM_GRAPHRAG_API_KEY"]
    print(f"embedding_api_key: {embedding_api_key}")

    llm_model = "Qwen3-32B"
    embedding_model = "text-embedding-v4"
    api_base = "http://10.10.22.76:4000/v1"
...
def find_chunks_keyword(
    ...
    """
    该函数find_chunks_keyword根据查询词，从预构建的FAISS索引中检索相关文本块。
    首先对查询进行嵌入并搜索最相关的词，再基于这些词找到对应的文本块，并按相似度排序。
    最后按 token 数量限制筛选结果，返回格式化后的相关文本。
    参数:
        query (str): 用户输入的查询语句。
        embedder (OpenAIEmbedding): 用于生成文本嵌入的模型对象。
        encoder (tiktoken.Encoding): 用于统计token数量的编码器。
        index (faiss.IndexFlatIP): 预构建的FAISS索引，用于词级别的相似性搜索。
        words (list): 与索引中向量一一对应的词列表。
        word_chunk_data (dict): 每个词对应的相关文本块信息，包括块ID、嵌入和token数。
        candidate_units_dict (dict): 文本单元字典，键为单元ID，值为TextUnit对象。
        chunks_size (int): 返回文本的最大token数限制。

    返回:
        str: 包含相关文本块的内容，按相似度排序并格式化后的字符串。
    """
    if chunks_size == 0:
        return "\n\n-----Text source that may be relevant-----\n\nN/A\n"
    query_embedding = embedder.embed(query)
...
def split_chunks_and_others(chunks: list[TextUnit], entities: list[Entity], relations: list[Relationship], token_encoder, new_size=150
    ...
    for chunk in chunks:
        ...
        for idx, text in enumerate(split_texts):
            ...
            new_chunk.text = text
            new_chunk.id = f"{new_chunk.id}_{idx}" # 设置新文本内容和新ID(原ID+序号)
