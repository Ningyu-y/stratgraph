...
from indexing_sket.cluster_sampling import cluster_based_sampling
from indexing_sket.visualize_sampling import visualize_sampling_distribution
...
async def filter_text_units(
    ...
    n_clusters = chunk_strategy.get('n_clusters', 500) 
    random.seed(42)
    ...
    elif build_skeleton_method == "cluster":
        print("使用聚类进行采样")
        sampled_text_units = cluster_based_sampling(
            text_units=text_units,
            sample_length=sample_length,
            n_clusters=n_clusters,
            knn_edges=knn_edges,
            
            build_knn_chunk_graph=build_knn_chunk_graph,
            nx_pagerank_func=nx.pagerank 
        )
    else:
        print("使用page rank进行采样")
...
def build_knn_chunk_graph(text_units: list[TextUnit], k1: int, k2: int) -> nx.Graph:
    # find all text-text pairs 构建KNN图 根据词频和语义相似度构建边
