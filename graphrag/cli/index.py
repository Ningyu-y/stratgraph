...
if __name__ == "__main__":
    ...
    # 测试参数
    test_root_dir = Path("/home/ycc/paper/KET-RAG/graph-test1")  # 修改为您的测试目录
    memprofile = False
    skip_validation = False
    
    print(f"开始测试 index_cli，目录: {test_root_dir}")
    
    try:
        # 调用 index_cli 函数
        index_cli(
            root_dir=test_root_dir,
            verbose=True,  # 设置为 True 以查看详细输出
            resume=None,
            memprofile=memprofile,
            cache=True,
            reporter=ReporterType.RICH,  # 使用 Rich 进度报告
            config_filepath=None,
            emit=[TableEmitterType.Parquet],  # 输出 Parquet 格式
            dry_run=False,  # 设置为 True 可以只验证配置而不实际运行
            skip_validation=skip_validation,
            output_dir=None,
        )
        print("测试完成")
